...
Here we see more agreement between model and obs, but the tendency for the model to underestimate ice coverage further from the pole persists. The exception being that the model thinks the ice shelf to the east of Greenland is much more consistent than the obs.
...
The same patterns seen throughout persist here, but the dissagreements along the edges of the ice fields are more dramatic, while the coverage over the pole is in bettere agreement (not surprisingly).
...
The differences in the patterns for variance are similar to the first moment stats above. The model shows less variance around the pole than the obs do. The model however tends to show more variance for ice near the edge of the shelf than the obs do. I'm not sure if this is an artifact of the models or the obs.
