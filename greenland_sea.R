...
GrS_summary <- OISST_NAPA_SST_summary %>% 
  # na.omit() %>% 
  left_join(lon_lat_NAPA_OISST, by = c("nav_lon", "nav_lat")) %>%
  mutate(lon_O_corrected = ifelse(lon_O > 180, lon_O-360, lon_O)) %>% 
  filter(lon_O_corrected <= GrS_bound[4], lon_O_corrected >= GrS_bound[3],
         lat_O <= GrS_bound[2], lat_O >= GrS_bound[1],
         min != 0,
         month == "daily") %>% 
...
GrS_grid_base <- GrS_summary %>% 
  ungroup() %>% 
  select(lon_O_corrected, lat_O) %>% 
  distinct() %>% 
  filter(lat_O > GrS_bound[1], lat_O < GrS_bound[2],
         lon_O_corrected > GrS_bound[3], lon_O_corrected < GrS_bound[4]) %>% 
  summarise_all(.funs = c("max", "min"))

GrS_grid <- expand.grid(seq(GrS_grid_base$lon_O_corrected_min, GrS_grid_base$lon_O_corrected_max, by = 0.25),
                       seq(GrS_grid_base$lat_O_min, GrS_grid_base$lat_O_max, by = 0.25)) %>% 
...
GrS_summary_interp <- GrS_summary %>% 
  group_by(product, month) %>% 
  nest() %>% 
  mutate(interp = map(data, interpp_data, df_grid = GrS_grid, interpp_stat = "mean")) %>% 
  select(-data) %>%
  unnest()
...
plot_1 <- ggplot(filter(ice_round, product == "OISST"), 
  ...
  geom_raster(data = filter(GrS_summary_interp, product == "OISST"), 
...
plot_2 <- ggplot(filter(ice_round, product == "NAPA"), 
                 aes(x = nav_lon, y = nav_lat)) +
  geom_raster(data = filter(GrS_summary_interp, product == "NAPA"), 
              aes(x = lon_O_corrected, y = lat_O, fill = interp)) +
  geom_contour(aes(z = ice_round), breaks = 0.25, colour = "white") +
  geom_contour(aes(z = ice_round), breaks = 0.5, colour = "grey 50") +
  geom_contour(aes(z = ice_round), breaks = 0.75, colour = "black") +
...
plot_3 <- ggplot(filter(ice_round, product != "difference"), 
                 aes(x = nav_lon, y = nav_lat)) +
  geom_raster(data = filter(GrS_summary_interp, product == "difference"),
              aes(x = lon_O_corrected, y = lat_O, fill = interp)) +
